...
public class SyncCrossAccountStage(IRcloneService rclone, ITranslationService localizer) : ISyncCrossAccountStage
{
    ...
    public async Task RunAsync(List<RemoteInfo> remotes, RemoteInfo master, IProgress<SyncProgressEvent> uiLogger, Action advanceProgress, CancellationToken cancellationToken)
    {
        ...
        foreach (var remote in remotes.Where(r => r.FolderId != master.FolderId))
        {
            ...
            var toCopy = sourceFiles
                .Where(f => f.IsConversation)
                .Where(srcFile => !masterConversations.TryGetValue(srcFile.Name, out var mFile) || srcFile.ModTime > mFile.ModTime.AddSeconds(2))
                .Select(f => f.Name)
                .ToList();
            ...
            if (toCopy.Any())
            {
                ...
                uiLogger.Report(new SyncProgressEvent(downloadTaskId, $"    ⬇ {string.Format(localizer["Log_Stage2_Download"], toCopy.Count, remote.FriendlyName)}", false));
                await rclone.ExecuteCommandAsync(new[] { "copy", sourcePath, masterPath, "--files-from-raw", "-", "--drive-server-side-across-configs" }, toCopy, cancellationToken, TimeSpan.FromMinutes(20));
        ...
        var distributeTasks = remotes.Where(r => r.FolderId != master.FolderId).Select(async remote => 
        {
            ...
            var toCopy = masterConversations.Values
                .Where(mFile => !destConversations.TryGetValue(mFile.Name, out var dFile) || mFile.ModTime > dFile.ModTime.AddSeconds(2))
                .Select(f => f.Name)
                .ToList();
            ...
            if (toCopy.Any())
            {
                ...
                uiLogger.Report(new SyncProgressEvent(uploadTaskId, $"    ⬆ {string.Format(localizer["Log_Stage2_Upload"], toCopy.Count, remote.FriendlyName)}", false));
                await rclone.ExecuteCommandAsync(new[] { "copy", masterPath, destPath, "--files-from-raw", "-", "--drive-server-side-across-configs" }, toCopy, cancellationToken, TimeSpan.FromMinutes(20));
